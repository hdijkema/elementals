...
#define DECLARE_EL_ARRAY(MODIFIER, NAME, T) \
  ...
  int NAME##_count(NAME a);

#define IMPLEMENT_EL_ARRAY(MODIFIER, NAME, T, COPY, DESTROY) \
  MODIFIER inline NAME NAME##_new(void)  { _el_array_new(void); } \
  MODIFIER inline el_array_code NAME##_insert(NAME a, int index, T *elem) { return _el_array_insert( a, index, (void *) COPY(elem)); } \
  MODIFIER inline el_array_code NAME##_append(NAME a, T *elem) { return _el_array_insert(a, NAME##_count(a), COPY(elem)); } \
  MODIFIER inline el_array_code NAME##_delete(NAME a, int index) { return _el_array_delete(a, index, ((void) (*)(void *)) DESTROY); } \
  MODIFIER inline el_array_code NAME##_set(NAME a,int index, T *elem) { return _el_array_set(a, index, COPY(elem), DESTROY); } \
